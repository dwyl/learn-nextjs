...
*route segments* that maps to a URL segment.
...
However, while *`layouts` persist state across routes*,
...
- **caching** is made *in-memory*, on the client side.
...
you can render a specific content to an *administrator* role
and a different one to a *user* role.
